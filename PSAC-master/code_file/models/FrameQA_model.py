...
class FrameQAModel(nn.Module):
    ...
    def forward(self, v, q_w, q_c, labels, dictionay, return_attns=False):
        # visual info
        vid_output = self.vid_encoder(v)  # v : batch_size x v_len x ctx_dim  # batch_size x v_len x d_v
        vid_output = v + vid_output
        # vid_output = v
        fus_output = self.ques_encoder(vid_output, q_w, q_c, labels)
...
def build_temporalAtt(task_name, n_layer, num_ans_candidates, num_hid, dictionary, glove_file):
    # w_emb = WordEmbedding(dataset.dictionary.ntoken, len(dataset.dictionary.idx2char), 300, 64, 0.0)
    vid_encoder = Encoder(n_layer=n_layer, n_head=8, d_k=64, d_v=64, v_len=36, v_emb_dim=300,
                               d_model=4096, d_inner_hid=512, dropout=0.1)
