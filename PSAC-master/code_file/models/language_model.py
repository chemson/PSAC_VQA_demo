...
class WordEmbedding(nn.Module):
    ...
    def init_embedding(self,dict, glove_file, task):
        '''
        build the embedding matrix of glove_6B_300d and save it in .npy file
        or load the embedding matrix from the .npy file
        :param dict: the dictionary of the words and the chars, it contains labels form cache/XXX.plk
                     the dimension of word label is 7150
                     the dimension of char label is 42?
                     if the word dimension changes, the model may need retrain.
        :param glove_file: the glove file's path
        :param task: task name, such as FrameQA, Count ...
        :return:
        '''
        if not os.path.exists('/mnt/project_vqa_2/PSAC-master1.0/code_file/data/%s_glove6b_init_300d.npy'%task):
            print('Construct initial embedding...')
            weight_init, word2emb = create_glove_embedding_init(dict.idx2word, glove_file)
            np.save(os.path.join('/mnt/project_vqa_2/PSAC-master1.0/code_file/data','%s_glove6b_init_300d.npy'% task), weight_init)
            ...
            np.save(os.path.join('/mnt/project_vqa_2/PSAC-master1.0/code_file/data','%s_char_glove6b_init_300d.npy'% task), weight_init_char)
        else:
            print('loading glove from /mnt/project_vqa_2/PSAC-master1.0/code_file/data/%s_glove6b_init_300d.npy'%task)
            weight_init = torch.from_numpy(np.load('/mnt/project_vqa_2/PSAC-master1.0/code_file/data/%s_glove6b_init_300d.npy'%task))
            weight_init_char = torch.from_numpy(np.load('/mnt/project_vqa_2/PSAC-master1.0/code_file/data/%s_char_glove6b_init_300d.npy'%task))
...
class Encoder(nn.Module):
    def __init__(self, n_layer=6, n_head=8, d_k=64, d_v=64, v_len=36, v_emb_dim=300,
        ...
        self.pos_linear = nn.Linear(300, 4096)
...
class Ques_Encoder(nn.Module):
    ...
    def forward(self, vid_enc, q_w, q_c, dictionary):
        mask = ((torch.ones_like(q_w)* 0)!=q_w).float()
        mask_c = (torch.ones_like(q_c)*0 != q_c).float()

        q_w_emb = self_word_embedding(q_w, self.word_emb.weight, dictionary) # batch_size x q_len x w_dim
...
def self_word_embedding(question_word, word_embedding_matrix, dictionary):
    length = word_embedding_matrix.shape[0]
    question_word_embedding = np.zeros((1, 20, 300), dtype=np.float32)
    question_embedding_index = 0
    word2emb = {}
    for word_index in question_word[0]:
        if word_index >= length and word2emb == {}:
            print("Extra embedding begins......")
            start_time = time.time()
            with open("/mnt/project_vqa_2/PSAC-master1.0/code_file/data/glove.6B.300d.txt", 'r', encoding='UTF-8') as f:
                entries = f.readlines()
            ...
            end_time = time.time()
            print("Extra embedding finished! This extra embedding cost ", end_time - start_time, " seconds.")
    for word_index in question_word[0]:
        if word_index < length:
            question_word_embedding[0, question_embedding_index, :] = word_embedding_matrix[word_index].cpu()
            question_embedding_index += 1
        else:
            word = dictionary.idx2word[word_index]
            if word not in word2emb:
                continue
            question_word_embedding[0, question_embedding_index, :] = np.array(word2emb[word])
            question_embedding_index += 1
    question_word_embedding = torch.from_numpy(question_word_embedding).cuda()
    return question_word_embedding
