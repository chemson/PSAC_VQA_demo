...
def train(model, train_loader, eval_loader, num_epochs, output, max_len=35):
    ...
    optim = torch.optim.Adam(model.parameters(), weight_decay=0.1)  #SGD(, lr=0.05, momentum=0.9)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q_w, q_c, a, b, c, d) in enumerate(train_loader):
            ...
            q_c = Variable(q_c).cuda(device) #cuda())
            v = np.array(v)
            v = np.tile(v, [1, model.num_choice]).reshape(-1,v.shape[-2], v.shape[-1])
            v = Variable(torch.from_numpy(v)).cuda(device) #cuda()
            q_w = Variable(q_w).cuda(device) #cuda())
            # a = a.type(torch.LongTensor)
            if model.model_name=='Count':
                a = Variable(a.float()).cuda(device) #cuda())
            else:
                a = Variable(a).cuda(device) #cuda())
            # a = a.to(torch.int32)
